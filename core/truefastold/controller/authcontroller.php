...
if (isset($_POST['LoginRequest'])) {
 ...
 if ($AuthToken == SECURE($_POST['AuthToken'], "d")) {
  ...
  if ($CheckUsername == true) {
   ...
   //check user type 
   if ($UserType == "Admin") {
    LOCATION("success", "Welcome $UserName, Login Successful!", DOMAIN . "/admin/index.php");
   } elseif ($UserType == "SERVICE_EXECUTIVE") {
    LOCATION("success", "Welcome $UserName, Login Successful!", DOMAIN . "/se/index.php");
   }
  } else {
 ...
 if ($UserPassword != $UserPassword_2) {
  ...
  $update = UPDATE("UPDATE users SET password='$UserPassword' where user_id='" . LOGIN_UserId . "'");
